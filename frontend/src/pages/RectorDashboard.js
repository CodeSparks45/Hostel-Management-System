...
export default function RectorDashboard() {
  ...
  // ─── REAL-TIME SYNC ENGINE (FOR PRESENTATION MAGIC) ───
  const [requests, setRequests] = useState([]);

  useEffect(() => {
    // This fetches requests saved in browser memory by VerifyPayment.js
    const syncRequests = () => {
      const storedRequests = JSON.parse(localStorage.getItem("pendingRequests")) || [
        // Default dummy requests if local storage is empty
        { id: "DUJ192837", name: "Dr. Anjali Patil", designation: "HOD, Computer Science", date: "Today, 10:30 AM", receipt: "sbi_receipt_1.pdf" },
        { id: "DUJ998273", name: "Guest: Mr. Sharma", designation: "AICTE Inspector", date: "Today, 11:15 AM", receipt: "sbi_receipt_2.pdf" }
      ];
      setRequests(storedRequests);
    };

    syncRequests();
    // Check for new requests every 2 seconds (Cross-Tab Magic!)
    const interval = setInterval(syncRequests, 2000); 
    return () => clearInterval(interval);
  }, []);

  // Faculty/Guest Master Logbook
  const [guests, setGuests] = useState([
    { id: "EMP-2041", name: "Prof. Arvind Kumar", designation: "Faculty", block: "Sahyadri", room: "A-102", status: "In Campus" },
    { id: "GST-9921", name: "Mr. Ramesh Desai", designation: "Official Guest", block: "VIP Wing", room: "V-01", status: "In Campus" },
    { id: "EMP-1102", name: "Dr. Sneha Rao", designation: "Principal Office", block: "Nandgiri", room: "N-204", status: "On Leave" },
  ]);

  // Room Matrix
  const [rooms, setRooms] = useState([
    { number: "A-101", type: "Faculty Suite", occupied: 1, capacity: 1, status: "Full" },
    { number: "A-102", type: "Faculty Suite", occupied: 1, capacity: 1, status: "Full" },
  ...
  const handleApprove = (id) => {
    toast.success("Pass Approved! Room Allocated ✅", { style: { borderRadius: '12px', background: '#ecfdf5', color: '#059669', fontWeight: 'bold' }});
    const updatedReqs = requests.filter(req => req.id !== id);
    setRequests(updatedReqs);
    localStorage.setItem("pendingRequests", JSON.stringify(updatedReqs)); // Update storage
  };

  const handleReject = (id) => {
    toast.error("Pass Rejected!", { style: { borderRadius: '12px', background: '#fff1f2', color: '#e11d48', fontWeight: 'bold' }});
    const updatedReqs = requests.filter(req => req.id !== id);
    setRequests(updatedReqs);
    localStorage.setItem("pendingRequests", JSON.stringify(updatedReqs)); // Update storage
  };
  ...
  return (
    <div className="min-h-screen bg-sky-50/50 font-sans flex text-slate-800">
      ...
      {/* ─── PREMIUM SIDEBAR (Restored Colorful Style) ─── */}
      ...
      <main className="flex-1 lg:ml-[260px] p-6 lg:p-10 max-w-7xl mx-auto w-full">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="h-12 w-12 bg-sky-100 text-sky-600 rounded-2xl shadow-sm border border-sky-200 flex items-center justify-center font-black text-lg">
              SD
            </div>
          </div>
        </header>
        ...
        <AnimatePresence mode="wait">
          ...
          {/* ════════ VIEW 1: DASHBOARD OVERVIEW (Restored Premium Bento) ════════ */}
          {activeView === "Overview" && (
            <motion.div key="overview" variants={fadeUp} initial="hidden" animate="show" exit={{ opacity: 0, y: -20 }}>
              
              {/* Colorful Stat Cards */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                <StatCard label="In-House Guests" value="42" icon={<UserCheck size={24} className="text-sky-500"/>} color="bg-sky-50 border-sky-100" />
                ...
                <StatCard label="VIP Suites Free" value="03" icon={<DoorOpen size={24} className="text-emerald-500"/>} color="bg-emerald-50 border-emerald-100" />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="p-6 md:p-8 space-y-4 flex-1">
                    {requests.slice(0,3).map((req, i) => (
                      <div key={i} className="flex items-center justify-between p-5 border border-slate-100 rounded-2xl hover:border-sky-200 hover:shadow-md transition-all group bg-slate-50/50">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-amber-100 text-amber-600 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform"><FileText size={20}/></div>
                          <div>
                        ...
                        <button onClick={() => setActiveView("Requests")} className="bg-sky-500 text-white px-5 py-2.5 rounded-xl text-xs font-bold shadow-md shadow-sky-500/20 hover:bg-sky-600 transition-all">Review</button>
                      </div>
                    ))}
                    {requests.length === 0 && <p className="text-center font-bold text-slate-400 py-8">Inbox Zero. No pending requests!</p>}
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-[2.5rem] p-8 text-white shadow-xl relative overflow-hidden flex flex-col justify-center">
                  ...
                  <p className="text-sm text-slate-300 mb-8 leading-relaxed">Guard scanner network is online. Access logs are syncing in real-time.</p>
                  <button onClick={() => toast("Ping successful!")} className="bg-sky-500 hover:bg-sky-400 text-white py-4 rounded-xl font-bold text-sm transition-all shadow-lg shadow-sky-500/30">
                    Ping Main Gate
                  </button>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[2rem] border border-sky-100 shadow-sm overflow-hidden">
                <div className="p-6 md:p-8 border-b border-slate-100 flex justify-between items-center bg-white">
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Approve passes for Faculty & Guests</p>
                  </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50">
                      <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">
                        <th className="px-8 py-5">DU Reference</th>
                    ...
                    <tbody className="text-sm font-bold text-slate-700">
                      {requests.length === 0 ? (
                        <tr><td colSpan="5" className="text-center py-20"><CheckCircle size={48} className="mx-auto mb-4 text-emerald-200"/><p className="text-emerald-500 font-extrabold text-lg">Queue Empty</p></td></tr>
                      ) : (
                        requests.filter(r => r.name.toLowerCase().includes(searchQuery.toLowerCase()) || r.id.toLowerCase().includes(searchQuery.toLowerCase())).map((req) => (
                          <tr key={req.id} className="border-b border-slate-50 hover:bg-sky-50/50 transition-colors">
                            <td className="px-8 py-5 font-mono text-xs text-slate-500">{req.id}</td>
                            <td className="px-8 py-5">
                              <p className="font-extrabold text-slate-800 text-base">{req.name}</p>
                              <p className="text-[10px] text-slate-400 uppercase tracking-widest mt-0.5">{req.designation}</p>
                            </td>
                            <td className="px-8 py-5 text-xs text-slate-500 font-medium">{req.date}</td>
                            <td className="px-8 py-5">
                              <button onClick={() => toast("Downloading Receipt...")} className="flex items-center gap-2 text-sky-600 bg-sky-50 px-3 py-1.5 rounded-lg border border-sky-100 font-bold text-xs hover:bg-sky-100">
                                <Download size={14}/> PDF
                              </button>
                            </td>
                            <td className="px-8 py-5 flex justify-end gap-3">
                              <button onClick={() => handleApprove(req.id)} className="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-emerald-500/20 flex items-center gap-1"><Check size={14}/> Approve</button>
                              <button onClick={() => handleReject(req.id)} className="px-5 py-2.5 bg-white border border-rose-200 text-rose-500 hover:bg-rose-50 rounded-xl text-xs font-bold transition-all flex items-center gap-1"><X size={14}/> Reject</button>
                            </td>
                          </tr>
                        ))
                      )}
               ...
               <div className="bg-white rounded-[2rem] border border-sky-100 shadow-sm overflow-hidden">
                  <div className="p-6 md:p-8 border-b border-slate-100 flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Official Registry</p>
                    </div>
                    <button onClick={() => toast("Exporting CSV...")} className="bg-slate-800 text-white px-5 py-3 rounded-xl text-xs font-bold flex items-center gap-2 hover:bg-slate-900 shadow-md">
                      ...
                      {guests.filter(g => g.name.toLowerCase().includes(searchQuery.toLowerCase())).map((guest, i) => (
                        <tr key={i} className="border-b border-slate-50 hover:bg-sky-50/50 transition-colors">
                          ...
                          <td className="px-8 py-5">
                            ...
                            <span className="text-[10px] text-slate-400 uppercase tracking-widest ml-2">({guest.block})</span>
                          </td>
                          <td className="px-8 py-5">
                            <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border ${
                              guest.status === 'In Campus' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'
                            }`}>{guest.status}</span>
                          </td>
                        </tr>
                      ))}
               ...
               <div className="bg-white rounded-[2rem] border border-sky-100 shadow-sm p-8">
                  <div className="flex justify-between items-center mb-8 border-b border-slate-100 pb-6">
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex items-center gap-2 text-xs font-bold text-slate-500"><span className="w-3 h-3 rounded-full bg-slate-100 border border-slate-200"></span> Occupied</div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    {rooms.map((room, i) => (
                      <div key={i} onClick={() => toast(`Opening config for ${room.number}`)} className={`cursor-pointer rounded-2xl p-6 border-2 transition-all hover:scale-[1.02] ${
                        room.status === 'Available' ? 'bg-emerald-50/50 border-emerald-100' :
                        room.status === 'Full' ? 'bg-slate-50 border-slate-200' : 'bg-rose-50/50 border-rose-100'
                      }`}>
                        <h4 className="text-xl font-black text-slate-800 mb-4">{room.number}</h4>
